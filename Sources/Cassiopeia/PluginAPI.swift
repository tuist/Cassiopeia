//===----------------------------------------------------------------------===//
//
// This source file is part of the Cassiopeia project
//
...
@preconcurrency import CassiopeiaPluginAPI
...
public func llcas_cas_create(
    ...
    let cas = Cassiopeia.makeRemoteCAS(baseURL: url)
    Task {
...
public func llcas_actioncache_put_for_digest(
    ...
    let keyBuffer = UnsafeBufferPointer(start: keyData, count: key.size)
